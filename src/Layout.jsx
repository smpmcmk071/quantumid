...
import { LayoutDashboard, Users, UserPlus, Target, LogOut, Menu, X, Briefcase, TrendingUp, User, Shield } from 'lucide-react';
...
export default function Layout({ children, currentPageName }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      // Auto-redirect users without profile to quantum profile page
      if (u && currentPageName !== 'UserQuantumProfile') {
        const profiles = await base44.entities.QuantumProfile.filter({ user_id: u.id });

        if (profiles.length === 0) {
          window.location.href = createPageUrl('UserQuantumProfile');
        }
      }
    };
    loadUser();
  ...
  const navItems = [
    { name: 'Quantum Profile', page: 'UserQuantumProfile', icon: Shield }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950">
      ...
      {currentPageName !== 'Marketing' && (
        ...
        <div className="max-w-7xl mx-auto px-4 py-1 flex items-center justify-between">
          <Link to={createPageUrl('UserQuantumProfile')} className="flex items-center">
